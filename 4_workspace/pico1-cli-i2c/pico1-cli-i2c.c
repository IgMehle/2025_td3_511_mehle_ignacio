...
#define I2C_SDA_PIN     2      // SDA1
#define I2C_SCL_PIN     3      // SCL1
...
int main() {
    ...
    while (true) {
        ...
        int len = (int)strlen(tx_buffer);
        int r = i2c_write_blocking(I2C_PORT, SLAVE_ADDR,
                                   (uint8_t*)tx_buffer, len, false);

        if (r < 0) {
            printf("[ERR] Fallo la escritura I2C (code=%d)\n", r);
            continue;
        }

        // ---- READ desde el SLAVE ----
        sleep_ms(20);  // pequeño delay para que el slave prepare su reply
        ...
        if (read_len < 0) {
            printf("[ERR] No se recibió respuesta del slave (code=%d)\n", read_len);
        } else {
