...
class GUIController:             
    ...
    def show_error(self, message):
        ...
        #QMessageBox.critical(None, "Error", message)       descomentar después
    ...
    def wait_for_ack(self):
        ...
        try:
            while True:
                ok, out = self.ssh_manager.execute_command(f"cat {self.char_device}")
                if ok and out:
    ...
    def enviar_datos(self):
        ...
        if self.ui.checkBox.isChecked():
            ...
            print("⚠ Selecciona una curva (Rápida o Lenta)")      
            return

        # Se arma comando:
        cmd = f"set lux -v {val} -h {vmax} -l {vmin} -a {curva}"

        #Se escribe en textbox
        self.ui.textEdit_log.append(cmd)

        # Se escribe en Raspberry:  
        self.ssh_manager.execute_command(f'echo -n {''} > {self.recepcion}')
    ...
    def leer_datos(self):
        ...
        cmd_getlux = "get lux"
        self.ssh_manager.execute_command(f'echo -n {''} > {self.recepcion}')
        self.ssh_manager.execute_command(f'echo "{cmd_getlux}" >> {self.char_device}')

        line = self.wait_for_line(timeout=5)    #Espera 5 segundos antes de salir
        if line is None:
            self.ui.textEdit_log.append("[TIMEOUT] Sin respuesta")
            return
        
        self.ui.textEdit_log.append("Lectura:\n")
        ...
        if not line.startswith("[NOW]"):   #Se va a tener que cambiar despues porque hay más comandos
            print("Comando inválido en recepción")   
            return
    
        try:
            partes = line.split()

            valor = partes[partes.index("LUX") + 1]
            valor_max = partes[partes.index("MAX") + 1]
            valor_min = partes[partes.index("MIN") + 1]
            curva = partes[partes.index("CURVA") + 1]
    ...
    def leer_log(self):
        ...
        self.ssh_manager.execute_command(f'echo -n {''} > {self.recepcion}')
        self.ssh_manager.execute_command(f'echo "{cmd_getlog}" >> {self.char_device}')

        if not self.wait_for_ack():
            self.ui.textEdit_log_append("[TIMEOUT] No llegó ACK")
            return
               

        stdout, stderr = self.ssh_manager.execute_command(f'cat {self.log}')      #stdout es un tuple: (True/False , "output"), stderr es el texto de salida y setPlainText() solo acepta string.
    ...
    def limpiar_memoria(self):
        ...
        cmd_clear = "set eclear"
        #open(self.recepcion, "w").close()
        self.ssh_manager.execute_command(f'echo -n {''} > {self.recepcion}')      #Verdadera limpieza de buffer
        self.ssh_manager.execute_command(f'echo "{cmd_clear}" >> {self.char_device}')
        self.ui.textEdit_log.append("El contenido de la EEPROM ha sido borrada")
        print("COMANDO SET ECLEAR ENVIADO OK")
